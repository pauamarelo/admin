...
"use strict";angular.module("myApp").controller("noticiasCtrl",function($scope,$http,config,toaster){var vm=this;function listar(){vm.isLoading=!0,$http.get(config.listNews).then(function(response){response.data.status&&(vm.res=response.data,vm.news=response.data.data,vm.isLoading=!1)}).catch(function(error){vm.res=error.data,vm.isLoading=!1})}vm.dados={},vm.img={},vm.isLoading=!1,vm.imgIsLoading=!1,$http.get(config.listarIntegrantes).then(function(response){vm.integrantes=response.data.data}),vm.orderByField="-createdAt",vm.reverseSort=!1,vm.pageSize="10",vm.pagination={current:1},listar(),vm.openAdd=function(){vm.dados={},$("#modalAdd").modal("show")},vm.adicionar=function(d){d.imagem?vm.imagem=vm.img.data.filePath:vm.imagem="";var objData={titulo:d.titulo,conteudo:d.conteudo,autor:d.autor,url:d.titulo.replace(/\W+/g,"-").toLowerCase(),img:vm.imagem,ativo:d.ativo};$http.post(config.news,objData).then(function(response){response.data.status?(toaster.success("Sucesso!",response.data.msg),listar(),$("#modalAdd").modal("hide")):toaster.error("Erro","Houve um erro.")})},vm.openEdit=function(d){vm.dados=d,vm.img={},$("#modalEdit").modal("show")},vm.editar=function(d){d.imagem?vm.imagem=vm.img.data.filePath:vm.imagem=d.img;var objData={titulo:d.titulo,conteudo:d.conteudo,autor:d.autor,url:d.titulo.replace(/\W+/g,"-").toLowerCase(),img:vm.imagem,ativo:d.ativo};$http.put(config.news+"/"+d._id,objData).then(function(response){response.data.status?(toaster.success("Sucesso!",response.data.msg),listar(),$("#modalEdit").modal("hide")):toaster.error("Erro","Houve um erro.")})},vm.ativo=function(post){post.ativo?post.ativo=0:post.ativo=1,vm.editar(post)},vm.delete=function(d){confirm("Você realmente deseja excluir este registro?")&&$http.delete(config.news+"/"+d._id).then(function(response){response.data.status?(toaster.success("Sucesso!",response.data.msg),listar()):toaster.error("Erro","Houve um erro.")})},vm.upload=function(d){vm.imgIsLoading=!0;var file=d,fd=new FormData;fd.append("imagem",file),$http.post(config.upload,fd,{transformRequest:angular.identity,headers:{"Content-Type":void 0}}).then(function(response){response.data.status?(toaster.success("Sucesso!",response.data.msg),vm.img=response.data):toaster.error("Erro",response.data.msg),vm.imgIsLoading=!1}).catch(function(error){toaster.error("Erro","Houve um erro"),vm.imgIsLoading=!1})},vm.detalhes=function(d){vm.dados=d,$("#modalDetalhes").modal("show")}});
